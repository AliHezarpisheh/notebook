...
![Terminal Emulator to Bash](../images/terminal-emulator-to-shell.png)
...
> Global bash configs are stored at `/etc/profile`. Each user has her configuration at
...
find . -size 1033c ! -executable -exec bash -c \
  'file "$1" | grep -i "ASCII text"' _ {} \;

find . -size 1033c ! -executable -exec sh -c \
  'file "$1" | grep -q "ASCII text"' _ {} \; -print
```

I believe you know `cat` and `cd`.

> There is a black hole in linux systems: `/dev/null`. Everything you send to it
> vanishes.
