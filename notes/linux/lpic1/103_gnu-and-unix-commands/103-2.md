...
will result in a complete different hash.

We use hashing and hashes for checking the integrity of data(or probably anything else
like an executable file or an ISO image of an OS). Also we use them for password
management in the software engineering realm. They have more use cases too.
...
How to send those annoying `permission denied` errors when using `find`? Let me
introduce you to file descriptors and the redirection syntax.

```bash
find / -type d -name "idk" 2> /dev/null  # redirect stderr to the void
```

`>` is the redirection operator. The `2` in the above command is a file descriptor
saying that redirect the stderr to another place. The file descriptors are integer
pointers that processes use to access open files or I/O streams. When a program starts,
it automatically gets three:

- 0 (stdin) - where input comes from.
- 1 (stdout) - where normal output goes.
- 2 (stderr) - where errors go.

Think of them as numbered channels for data flow.

> `>` means `1>`. It means redirect stdout.
