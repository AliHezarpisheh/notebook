...
MBR is only 512 bytes which is not sufficient for new boot loaders. Some versions of
Lilo could fit in that place. That sector would then load a smarter bootloader(e.g.
grub2) to load the kernel and initramfs.
...
> 💡 Chain loading is when a bootloader loads another bootloader. This is done when a
...
> 💡 You can use `update-grub` for a front-end of above commands. `/etc/grub.d/` files
> are each a part of the main `/boot/grub/grub.cfg`.

#### Simple Example of menu.lst
