...
- It also do traffic offloading.
  - Handle requests for static contents and distribute the load into multiple servers.
  - Let the origin server focuses on dynamic content generation and application logic.

**What are the CDN use cases?** Every website that use to deliver content to global
audience with minimal latencies uses CDNs. Examples are: Streaming platforms(e.g.
Netflix), news agencies, e-commerce websites.

## Additional Devices Functionality

### Virtual Private Networks(VPNs)

A stream of network packets that are hidden inside of other packets is called VPN. The
outer packets acts as transport mechanism, carrying the inner packets to their
destination. The hidden packets are typically encrypted and the primary use for VPNs are
for security, privacy, or evasion.

> 🔒 Firewalls, routers, and anonymizers can be used to create/terminate VPNs.

In most cases, the public clients connect to the anonymizers via VPN.

### Quality of Service(QoS)

Quality of Service refers to a set of network management techniques that prioritize the
certain type of traffics. QoS assigns a priority level to the traffic based on the type(
voice, video, data). QoS classification can be performed by a router, a switch, or even
an application. Obviously, the routers and switches along the way enforce the QoS
prioritization and because of that, the QoS markings are not honored between Telcos and
ISPs.

QoS will ensure optimal performance for critical applications. It minimizes latency,
jitter, and packet-loss for time-sensitive applications and it ensures high-priority
traffics(video streaming, VoIP, etc) receives sufficient resource even during network
congestion.

> 🏭 Depending on the model, routers, switches, firewalls, load balancers, and proxies
> all support QoS.
...
This is an IP header packet value which specifies how many hops (routers) can an IP
packet traverse before getting discarded in the network. We certainly don't want an IP
be forever in our network or stuck in an indefinite loop forever, so every time a IP
packet is received by a router, the router will decrement the ttl header by 1. When a
router decremented the ttl header to zero, it will discard the packet and send an ICMP
Time Exceed message back to the sender.

> ➡ TTL value is set by the host. The default value of ttl header for CISCO, Windows,
> and Linux are 255, 128, and 64, respectively.

#### traceroute

`traceroute` is an application which uses the ttl functionality to identifies the
routers all along to the host. It will send a packet with `ttl=1` header to identifies
the first router, then a packet with `ttl=2` to identifies the second router, and do
this all along to reach the host.

More technically, traceroute will sends packets with increasing tll values to map the
taken path by packets. This is done to identify potential network issues.
